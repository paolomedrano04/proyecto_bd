...
def generate_item_vendido(n, productos, ventas):
    combinaciones_usadas = set()
    for _ in range(n):
        while True:
            producto = random.choice(productos)
...
personas = generate_persona(3000)
clientes = generate_cliente(2000, personas)
repartidores = generate_repartidor(1000, [p for p in personas if p not in clientes], [e[0] for e in datos_fijos['empresas'] if e[4] == 'Courier'])
ordenes = generate_orden_de_compra(1000, [e[0] for e in datos_fijos['empresas']], [d[0] for d in datos_fijos['despachos']])
comprobantes = generate_comprobante_de_pago(1000)
ventas = generate_venta(1000, repartidores, personas, comprobantes)
generate_stock(500, productos, [d[0] for d in datos_fijos['despachos']])
generate_abastecimiento(1000, productos, ordenes)
generate_item_vendido(1000, productos, ventas) 
generate_pago(1000, ventas, datos_fijos['trabajadores'])
